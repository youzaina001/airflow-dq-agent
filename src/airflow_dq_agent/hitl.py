...
def _actor_field(value: object, key: str) -> str | None:
    field = value.get(key) if isinstance(value, Mapping) else getattr(value, key, None)
...
def parse_approval_output(output: Mapping[str, Any], *, approver_ids: Set[str]) -> HumanDecision:
    ...
    actor_id = _actor_field(responder, "id")
    actor_name = _actor_field(responder, "name")
